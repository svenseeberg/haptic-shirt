...
import sys, os, serial, threading, string, random, time
try:
...
---    i          start interactive type->vib mode
---    v          start tippVib(c) mode
""" % {
            ...
            while self.alive:
                ...
                if menu_active:
                    if c == MENUCHARACTER or c == EXITCHARCTER: # Menu character again/exit char -> send itself
                        ...
                        self.dump_port_settings()
                    elif c in 'vV':                         # v -> tippvib mode
                        sys.stderr.write('--- tippVib(c) mode, press any key to start, exit with escape ---\n')
                        tippvib_active = True
                    else:
                ...
                # TIPP_HANS
                    ...
                    if c == '\x1b':
                        ...
                        tippvib_active = False
                        counter = 0
                        for i in range(0, len(tv_read_buffer)):
                            if tv_read_buffer[i] != tv_write_buffer[i]:
                                counter += 1
                        if len(tv_read_buffer) != 0:
                            percent = float(counter) / float(len(tv_read_buffer))
                            percent = percent * 100
                            tomtom =  time.time() - tv_start_time
                            tomtom = float(tomtom) / float(60)
                            tomtom = float(len(tv_read_buffer)) / float(tomtom)

                            tomtom =  round(tomtom, 2)
                            percent = round(percent, 2)
                        else:
                            percent = 0
                        sys.stderr.write('--- tippVib(c) stopped. Wrong keys: {0}%; {1} keys/min ---\n'.format(percent, tomtom))
                        tv_write_buffer = ""
                        tv_read_buffer = ""
                        tv_start_time = 0
                        tv_num_chars = 0
                        ...
                        if(tv_message == False):
                            sys.stderr.write('Please enter word length (0-9)')
                            tv_message = True
                        elif(c != ''):
                            tv_num_chars = string.atoi(c)
                            sys.stderr.write('You selected {0} characters.'.format(tv_num_chars))
                    else:
                        if tv_write_buffer != "":
                            tv_read_buffer += c
                            #index = len(tv_write_buffer) - 1
                            #check previous character
                            if tv_write_buffer[tv_write_pos] == c:
                                sys.stdout.write('%s\n' % c)
                                sys.stdout.flush()
                            else:
                                sys.stdout.write('You typed %s, correct answer is (%s)\n' % (c, tv_write_buffer[tv_write_pos]))
                            tv_write_pos = tv_write_pos + 1
                            if tv_write_pos == len(tv_write_buffer):
                                tv_get_chars = True
                        ...
                        #get random new character(s)
                        if tv_get_chars == True:
                            time.sleep(0.5)
                            tv_get_chars = False
                            for i in range(0,tv_num_chars):
                                write_char = random.choice(string.letters)
                                write_char = write_char.lower()
                                tv_write_buffer += write_char
                                self.serial.write(write_char)
                                time.sleep(0.75)
